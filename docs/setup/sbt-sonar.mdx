...
- **2.2.0** (05.07.2020) - Improve compatibility with
  ...
  plugin ([#117](https://github.com/mwz/sbt-sonar/pull/117)). This release adds
  a new setting key (`sonarExpectSonarQubeCommunityPlugin`), which allows
  toggling compatibility between [sonar-scala](https://sonar-scala.com) and
  [SonarScala](https://docs.sonarqube.org/latest/analysis/languages/scala)
...
- **2.0.0** (17.06.2019) - Use an embedded sonar-scanner
  ([#34](https://github.com/mwz/sbt-sonar/pull/34)) :confetti_ball: This version
  removes the dependency on having the standalone sonar-scanner-cli installed.
  To upgrade from 1.x please define the `sonar.host.url` property explicitly
  before running the `sonarScan` task (see the
