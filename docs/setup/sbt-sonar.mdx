...
- **sonar.scala.version** - defines the version of Scala used in your project
  (i.e. `scalaVersion`)
- **sonar.scala.coverage.reportPaths** - relative path to the scoverage report
  (e.g. `target/scala-2.12/scoverage-report/scoverage.xml` or `target/scoverage-report/scoverage.xml`)
- **sonar.scala.scapegoat.reportPaths** - relative path to the scapegoat report
  (e.g. `target/scala-2.12/scapegoat-report/scapegoat-scalastyle.xml` or `target/scapegoat-report/scapegoat-scalastyle.xml` )
...
- **2.2.0** (05.07.2020) - Improve compatibility with
  ...
  plugin ([#117](https://github.com/mwz/sbt-sonar/pull/117)).  
  This release adds a new setting key (`sonarExpectSonarQubeCommunityPlugin`),
  which allows toggling compatibility between
  [sonar-scala](https://sonar-scala.com) and
  [SonarScala](https://docs.sonarqube.org/latest/analysis/languages/scala)
...
- **2.1.1** (25.04.2020) - Fix missing sonar.projectVersion property
  ([#104](https://github.com/mwz/sbt-sonar/pull/104)).
...
- **2.0.0** (17.06.2019) - Use an embedded sonar-scanner
  ([#34](https://github.com/mwz/sbt-sonar/pull/34)) :confetti_ball:  
  This version removes the dependency on having the standalone sonar-scanner-cli
